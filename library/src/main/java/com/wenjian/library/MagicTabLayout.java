 ...
 */
public class MagicTabLayout extends HorizontalScrollView {
    ...
    /**
     * Gravity used to fill the {@link MagicTabLayout} as much as possible. This option only takes effect
    ...
    /**
     * Gravity used to lay out the tabs in the center of the {@link MagicTabLayout}.
     *
     * @see #setTabGravity(int)
     * @see #getTabGravity()
     */
    ...
    public MagicTabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.MagicTabLayout,
                defStyleAttr, R.style.Widget_Design_TabLayout);

        mTabStrip.setSelectedIndicatorHeight(
                a.getDimensionPixelSize(R.styleable.MagicTabLayout_tabIndicatorHeight, 0));

        mTabStrip.setSelectedIndicatorWidth(
                a.getDimensionPixelSize(R.styleable.MagicTabLayout_tabIndicatorWidth, -1));

        mTabStrip.setSelectedIndicatorOffset(
                a.getDimensionPixelSize(R.styleable.MagicTabLayout_tabIndicatorOffset, 0));


        mTabStrip.setSelectedIndicatorColor(a.getColor(R.styleable.MagicTabLayout_tabIndicatorColor, 0));

        mTabPaddingStart = mTabPaddingTop = mTabPaddingEnd = mTabPaddingBottom = a
                .getDimensionPixelSize(R.styleable.MagicTabLayout_tabPadding, 0);
        mTabPaddingStart = a.getDimensionPixelSize(R.styleable.MagicTabLayout_tabPaddingStart,
                mTabPaddingStart);
        mTabPaddingTop = a.getDimensionPixelSize(R.styleable.MagicTabLayout_tabPaddingTop,
                mTabPaddingTop);
        mTabPaddingEnd = a.getDimensionPixelSize(R.styleable.MagicTabLayout_tabPaddingEnd,
                mTabPaddingEnd);
        mTabPaddingBottom = a.getDimensionPixelSize(R.styleable.MagicTabLayout_tabPaddingBottom,
                mTabPaddingBottom);

        mTabTextAppearance = a.getResourceId(R.styleable.MagicTabLayout_tabTextAppearance,
        ...
        if (a.hasValue(R.styleable.MagicTabLayout_tabTextColor)) {
            // If we have an explicit text color set, use it instead
            mTabTextColors = a.getColorStateList(R.styleable.MagicTabLayout_tabTextColor);
        }

        if (a.hasValue(R.styleable.MagicTabLayout_tabSelectedTextColor)) {
            // We have an explicit selected text color set, so we need to make merge it with the
            // current colors. This is exposed so that developers can use theme attributes to set
            // this (theme attrs in ColorStateLists are Lollipop+)
            final int selected = a.getColor(R.styleable.MagicTabLayout_tabSelectedTextColor, 0);
            mTabTextColors = createColorStateList(mTabTextColors.getDefaultColor(), selected);
        }

        mRequestedTabMinWidth = a.getDimensionPixelSize(R.styleable.MagicTabLayout_tabMinWidth,
                INVALID_WIDTH);
        mRequestedTabMaxWidth = a.getDimensionPixelSize(R.styleable.MagicTabLayout_tabMaxWidth,
                INVALID_WIDTH);
        mTabBackgroundResId = a.getResourceId(R.styleable.MagicTabLayout_tabBackground, 0);
        mContentInsetStart = a.getDimensionPixelSize(R.styleable.MagicTabLayout_tabContentStart, 0);
        mMode = a.getInt(R.styleable.MagicTabLayout_tab_mode, MODE_FIXED);
        mTabGravity = a.getInt(R.styleable.MagicTabLayout_tab_gravity, GRAVITY_FILL);
        a.recycle();
    ...
    public void setSelectedTabIndicatorOffset(int offset) {
    ...
    /**
     * Add a {@link MagicTabLayout.OnTabSelectedListener} that will be invoked when tab selection
     * changes.
     *
    ...
    /**
     * Remove the given {@link MagicTabLayout.OnTabSelectedListener} that was previously added via
    ...
    /**
     * Remove all previously added {@link MagicTabLayout.OnTabSelectedListener}s.
     */
    ...
    /**
     * Returns the current mode used by this {@link MagicTabLayout}.
     *
     * @see #setTabMode(int)
     */
    @Mode
    ...
    /**
     * The one-stop shop for setting up this {@link MagicTabLayout} with a {@link ViewPager}.
     *
    ...
    private class SlidingTabStrip extends LinearLayout {
        ...
        void setSelectedIndicatorOffset(int offset) {
        ...
        public void draw(Canvas canvas) {
            ...
            if (mIndicatorLeft >= 0 && mIndicatorRight > mIndicatorLeft) {
                ...
                if (mSelectedIndicatorWidth != -1) {
                    left = mIndicatorLeft + (mIndicatorRight - mIndicatorLeft - mSelectedIndicatorWidth) / 2;
                    right = left + mSelectedIndicatorWidth;
                } else {
                    left = mIndicatorLeft + mSelectedIndicatorOffset;
                    right = mIndicatorRight - mSelectedIndicatorOffset;
                }
     ...
     * necessary calls back to the provided {@link MagicTabLayout} so that the tab position is
     * kept in sync.
     *
    ...
    public static class TabLayoutOnPageChangeListener implements ViewPager.OnPageChangeListener {
        private final WeakReference<MagicTabLayout> mTabLayoutRef;
        private int mPreviousScrollState;
        private int mScrollState;

        public TabLayoutOnPageChangeListener(MagicTabLayout tabLayout) {
        ...
        public void onPageScrolled(final int position, final float positionOffset,
            ...
            final MagicTabLayout tabLayout = mTabLayoutRef.get();
            if (tabLayout != null) {
    ...
    /**
     * A {@link MagicTabLayout.OnTabSelectedListener} class which contains the necessary calls back
     * to the provided {@link ViewPager} so that the tab position is kept in sync.
     */
    public static class ViewPagerOnTabSelectedListener implements MagicTabLayout.OnTabSelectedListener {
        ...
        @Override
        public void onTabSelected(MagicTabLayout.Tab tab) {
            mViewPager.setCurrentItem(tab.getPosition());
        }

        @Override
        public void onTabUnselected(MagicTabLayout.Tab tab) {
            // No-op
        }

        @Override
        public void onTabReselected(MagicTabLayout.Tab tab) {
            // No-op
        }
    }
